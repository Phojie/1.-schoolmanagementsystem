  ...
  <q-page class="q-pa-md-md">
    ...
    >
      <template v-slot:top="props">
        ...
        <q-avatar icon="print" class="print-hide q-ml-md-md cursor-pointer" @click="printStudents">
      ...
      <template v-slot:body="props">
        <q-tr :props="props" class="text-capitalize">
          <q-td class="text-bold" key="idnumber" :props="props"># {{ props.row.idnumber }}</q-td>
          <q-td key="profileImgUrl" :props="props">
            <q-avatar color="green-2" size="lg" rounded>
              <img
                style="padding:1px"
          ...
          <q-td
            key="fullname"
            :props="props"
          >{{props.row.prefix}} {{ props.row.fullname }} {{props.row.suffix}}</q-td>
          ...
          <q-td key="keyIndex" :props="props">
            <div class="text-blue-grey-6 full-width row wrap justify-center content-start">
              <q-avatar
                ...
                icon="remove_red_eye"
              >
                <q-tooltip>View {{ props.row.surname }}'s Profile</q-tooltip>
              </q-avatar>
    ...
    <q-page-sticky position="bottom-right" :offset="[18, 18]">
      <q-btn ref="testref" @click="addStudentData" fab icon="add" color="secondary" />
    </q-page-sticky>
...
export default {
  ...
  components: {
    personnelFormModal: require("components/shared/modals/personnelFormModal.vue")
      .default
  },
  data() {
    return {
  ...
  methods: {
    ...
    submitAddPersonnel() {
      ...
      if (this.$v.studentInformationForm.$invalid) {
        ...
        this.addPersonnelLists(this.studentInformationForm).then(
          function(result) {
            var fullname = `${
              vm.studentInformationForm.firstname
            } ${vm.studentInformationForm.middlename.charAt(0)}. ${
              vm.studentInformationForm.surname
            }`;
            vm.$q.notify({
    ...
    deleteStudent(data) {
      ...
      this.$q
        ...
        .onOk(() => {
          this.deletePersonnelLists(data).then(
            function(result) {
              vm.$q.notify({
    ...
    submitUpdatePersonnel() {
      if (this.$v.studentInformationForm.$invalid) {
        ...
        if (!this.jie1) {
          let vm = this;
          let jie2 = isEqualWith(
            this.dummyStudentInformationForm.profileImgUrl,
            this.studentInformationForm.profileImgUrl
          );
          if (jie2) {
            var falseUpload = {
            ...
            this.updatePersonnelLists(falseUpload).then(
              function(result) {
            ...
            this.updatePersonnelLists(trueUpload).then(
              function(result) {
                vm.updateStudentData(result);
                var fullname = `${
                  vm.studentInformationForm.firstname
