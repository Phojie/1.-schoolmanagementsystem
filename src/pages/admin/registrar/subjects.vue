    ...
    <div class="bg-secondary text-white">
      ...
      <q-toolbar inset>
        <q-breadcrumbs style="font-size: 15px" active-color="white">
          ...
          <q-breadcrumbs-el v-else-if="$route.query.semester === 'Summer'" class="text-amber" :label="$route.query.semester" icon="label" />
        </q-breadcrumbs>
    </q-toolbar>
    </div>
        ...
        <q-card >
          ...
          <q-card-section>
            <q-list>
              ...
              <q-item clickable @click="semesterGet('Summer', sy.schoolYear)">
                <q-item-section avatar>
                  <q-icon color="amber" name="label" />
                </q-item-section>

                <q-item-section>
                  <q-item-label>Summer</q-item-label>
                </q-item-section>
    ...
    <q-dialog
      ...
      >
        <subjectsForm
          ...
          :department.sync="dataInformation.department"
...
export default {
  ...
  methods: {
    ...
    deleteAct (data) {
      ...
      this.$q.dialog({
        title: 'Confirm',
        message: `Are you sure you want to delete this School Year <span class="text-green text-weight-bolder">${data.schoolYear} - ${this.syAdd(data.schoolYear)}</span>, All Subjects attached will be permanently deleted  `,
        cancel: true,
        html: true
      }).onOk(() => {
